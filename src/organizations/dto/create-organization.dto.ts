 ...
 * Used for creating a new organization within a tenant
 */
export class CreateOrganizationDto {
  @IsNotEmpty({ message: 'Organization key is required' })
  @IsString({ message: 'Organization key must be a string' })
  @MinLength(2, { message: 'Organization key must be at least 2 characters' })
  @MaxLength(128, { message: 'Organization key must not exceed 128 characters' })
  @Matches(/^[a-z0-9]([a-z0-9-]*[a-z0-9])?$/, {
    message:
      'Organization key must be lowercase alphanumeric with hyphens, and cannot start or end with a hyphen',
  })
